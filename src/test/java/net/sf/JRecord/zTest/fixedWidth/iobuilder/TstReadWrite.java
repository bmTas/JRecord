...
public class TstReadWrite  {
	...
	@Test public void testRead01() throws IOException  {
		doReadTest(defineByLength());
	}

		
	@Test public void testRead02() throws IOException  {
		doReadTest(defineByPosition());
	}

	@Test public void testWrite01() throws IOException  {
		doWriteTest(defineByLength());
	}

		
	@Test public void testWrite02() throws IOException  {
	...
	private void doReadTest(IFixedWidthIOBuilder iob) throws IOException {
		...
		while ((line = r.read()) != null) {
			for (int j = 0; j < FIELD_VALUES[i].length; j++) {
				assertEquals(line.getFieldValue(0, j).asString(), FIELD_VALUES[i][j], i + ", " + j);
			}
			i += 1;
		}
