...
public class TstCsvName1stLineLoader  extends JUnit3Test  {
	...
	@Test public void testLoadCopyBook1() throws IOException, RecordException {
		tst("\t");
	}
	
	
	@Test public void testLoadCopyBook2() throws IOException, RecordException {
		tst(",");
	}
	...
	private void tst(String sep) throws IOException, RecordException  {
		...
		assertEqualsV3i("Wrong number of records 1", 1, r.getNumberOfRecords());

		
		r = r.getRecord(0);
		assertEqualsV3i("Wrong number of records 0", 0, r.getNumberOfRecords());
		assertEqualsV3i("Wrong number of fields ", names.length, r.getNumberOfRecordFields());
		
		for (int i = 0; i < names.length; i++) {
			assertEqualsV3o("Wrong Field name " + i , names[i], r.getRecordField(i).getName());
		}
	}
