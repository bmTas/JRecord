 ...
 *
 */
public class TstComplicatedCsv  {
	...
	public void tstRead(boolean useCsvLine) throws IOException {
		...
		while ((l = r.read()) != null) {
			...
			for (int j = 1; j < 4; j++) {
				...
				if (idx == j) {
					assertEquals( CsvData.STRINGS[i], fieldValue.asString(), msg);					
				} else {
					assertEquals( j * 11, fieldValue.asInt(), msg);
				}
			}
			lastIdx = idx;
			i += 1;
	...
	public void tstWrite(boolean useCsvLine) throws IOException {
		...
		w.close();
		assertEquals(
				expected, Conversion.toString(os.toByteArray(), Conversion.DEFAULT_ASCII_CHARSET),
				useCsvLine + " ");
	}
}
