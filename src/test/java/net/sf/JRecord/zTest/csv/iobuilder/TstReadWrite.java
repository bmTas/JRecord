 ...
 *
 */
public class TstReadWrite  {

	
	@Test public void testReadComma() throws IOException {
		tst(',');
	}
	
	@Test public void testReadSemi() throws IOException {
		tst(';');
	}

	@Test public void testReadX00() throws IOException {
		tstBin((byte)0, "x'00'");
	}

	@Test public void testReadX01() throws IOException {
		tstBin((byte)1, "x'01'");
	}

	@Test public void testReadXFE() throws IOException {
		tstBin((byte)0xFe, "x'FE'");
	}

	@Test public void testWriteComma() throws IOException {
		tstWrite(',');
	}
	
	@Test public void testWriteSemi() throws IOException {
		tstWrite(';');
	}

	@Test public void testWriteX00() throws IOException {
		tstWriteBin((byte)0, "x'00'");
	}

	@Test public void testWriteX01() throws IOException {
		tstWriteBin((byte)1, "x'01'");
	}

	@Test public void testWriteXFE() throws IOException {
	...
	private void tstRead1(byte[] data, String sep, boolean csvLine, int io) throws IOException {
		...
		while ((l = reader.read()) != null) {
			for (int j = 0; j < CsvData.CsvLineData[i].length; j++) {
				assertEquals( CsvData.CsvLineData[i][j], l.getFieldValue(0, j).asString(), i + ", " + j);
			}
			i += 1;
		}
		assertEquals(CsvData.CsvLineData.length, i, "lines read");
	}
