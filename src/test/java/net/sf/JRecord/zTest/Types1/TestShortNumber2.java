...
public class TestShortNumber2 extends JUnit3Test  {
	...
	@Test public void testZdAsciiPositive() {
	...
	@Test public void testZdEbcdicPositive() {
	...
	@Test public void testShortZdUnsigned() {
		...
		for (int i = 0; i < 10; i++) {
			...
			assertEquals((long) i, typeAsciiZd.asUnscaledLong(Conversion.getBytes(str, ""), 1, field));
			assertEquals((long) i, typeAsciiZdP.asUnscaledLong(Conversion.getBytes(str, ""), 1, field));
			assertEquals((long) i, typeEbcdicZd.asUnscaledLong(Conversion.getBytes(str, "cp037"), 1, field));
			assertEquals((long) i, typeEbcdicZdP.asUnscaledLong(Conversion.getBytes(str, "cp037"), 1, field));
		...
		for (int i = 0; i < 10; i++) {
			...
			assertEquals((long) 10 + i, typeAsciiZd.asUnscaledLong(Conversion.getBytes(str, ""), 1, field));
			assertEquals((long) 10 + i, typeEbcdicZd.asUnscaledLong(Conversion.getBytes(str, "cp037"), 1, field));
			assertEquals((long) 10 + i, typeAsciiZdP.asUnscaledLong(Conversion.getBytes(str, ""), 1, field));
			assertEquals((long) 10 + i, typeEbcdicZdP.asUnscaledLong(Conversion.getBytes(str, "cp037"), 1, field));
	...
	@Test public void testShortZdUnsignedSet() {
		@SuppressWarnings("deprecation")
		FieldDetail field = FieldDetail.newFixedWidthField("", Type.ftZonedAsciiSmall, 1, 2, 0, "");
		...
		for (int i = 0; i < 100; i++) {
			...
			assertEqualsV3o(value, strP, Conversion.toString(typeEbcdicZdP.setField(new byte[2], 1, fielde, value), "cp037"));
	...
	@Test public void testShortZdSigned() {
		@SuppressWarnings("deprecation")
		FieldDetail field = FieldDetail.newFixedWidthField("", Type.ftZonedAsciiSmall, 1, 2, 0, "");
		TypeZonedAsciiSmall typeAsciiZd = new TypeZonedAsciiSmall(false);
		TypeZonedEbcdicSmall typeEbcdicZd = new TypeZonedEbcdicSmall(false);
		...
		for (int i = 0; i < 10; i++) {
			...
			assertEquals((long) i, typeAsciiZd.asUnscaledLong(Conversion.getBytes(str, ""), 1, field));
			assertEquals((long) i, typeAsciiZdP.asUnscaledLong(Conversion.getBytes("0" + i, ""), 1, field));
			assertEquals((long) i, typeEbcdicZd.asUnscaledLong(Conversion.getBytes(str, "cp037"), 1, field));
	...
	private void chkOneGetField(String id, byte[] record, TypeNum typeNum, ITypeBinaryExtendedNumeric typeSB,
		...
		assertEqualsV3(id, unscaledValue, typeSB.asUnscaledLong(record, field.getPos(), field));
	...
	private void chkGetField3(Random r, int randomScale, TypeNum typeNum, ITypeBinaryExtendedNumeric typeSB, int size, String charset) {
		...
		for (int i = 0; i < 300; i++) {
			...
			typeNum.setField(record, 1, field, bd);
			assertEqualsV3(randomScale + ": " + i, unscaledValue, typeSB.asUnscaledLong(record, field.getPos(), field));
			assertEquals(bd, typeSB.getField(record, field.getPos(), field));
	...
	private void chkSetField1(Random r, int randomScale, TypeNum typeNum, ITypeBinaryExtendedNumeric typeSB, String charset) {
		...
		for (int i = 0; i < 500; i++) {
			...
			assertEqualsV3(i + " " + bd + " " + f, unscaledValue, typeSB.asUnscaledLong(record, field.getPos(), field));
			assertEquals(s, typeNum.getField(record, field.getPos(), field));

			typeSB.setField(record, 1, field, s);
			assertEqualsV3(i + " ", unscaledValue, typeSB.asUnscaledLong(record, field.getPos(), field));
			assertEquals(s, typeNum.getField(record, field.getPos(), field));
	...
	private void chkSetField2(Random r, int randomScale, TypeNum typeNum, ITypeBinaryExtendedNumeric typeSB, String charset) {
		...
		for (int i = 0; i < 500; i++) {
			...
			if (! (typeSB instanceof TypeNum) || ! ((TypeNum) typeSB).isPositive()) {
				...
				typeSB.setField(record, 1, field, -d);
				assertEqualsV3(i + " " + bd + " " + d, -unscaledValue, typeSB.asUnscaledLong(record, field.getPos(), field));
				assertEquals(s, typeNum.getField(record, field.getPos(), field));
