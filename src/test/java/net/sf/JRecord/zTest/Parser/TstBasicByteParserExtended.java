...
public class TstBasicByteParserExtended extends JUnit3Test {
	...
	@Test public void testGetField1a() {
		ICsvByteLineParser p = new BasicCsvByteLineParserExtended(false);
		tstGetField1(p);
	}

	@Test public void testGetField1b() {
	...
	private void tstGetField1(ICsvByteLineParser p) {
		...
		for (int i = 1; i < lines1.length; i++) {
			...
			assertEqualsV3o("Error in " + i + "a, string was " + s, "f1" + i, s);
			s = p.getField(0, lines2[i].getBytes(), new CsvDefinition("|" , "'"));
			assertEqualsV3o("Error in " + i + "b, string was " + s, "f1" + i, s);
			s = p.getField(0, lines0[i].getBytes(), new CsvDefinition("," , ""));
			assertEqualsV3o("Error in " + i + "c, string was " + s, "f1" + i, s);

			s = p.getField(2, lines1[i].getBytes(), new CsvDefinition("," , "'"));
			assertEqualsV3o("Error in " + i + "d, string was " + s, "f3" + i, s);
			s = p.getField(2, lines2[i].getBytes(), new CsvDefinition("|" , "'"));
			assertEqualsV3o("Error in " + i + "e, string was " + s, "f3" + i, s);
			s = p.getField(2, lines0[i].getBytes(), new CsvDefinition("," , ""));
			assertEqualsV3o("Error in " + i + "f, string was " + s, "f3" + i, s);

		}
	}

	@Test public void testGetField2a() {
		ICsvByteLineParser p = new BasicCsvByteLineParserExtended(false);
		tstGetField2(p);
	}

	@Test public void testGetField2b() {
		ICsvByteLineParser p = new BasicCsvLineParserExtended(false);
		tstGetField2(p);
	}
	...
	private void tstGetField2(ICsvByteLineParser p) {
		...
		assertEqualsV3o("Error in 11a, string was " + s, "pt1, pt2, pt3", s);
		s = p.getField(1, lines2[1].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 11b, string was " + s, "pt1| pt2| pt3", s);


		s = p.getField(1, lines1[2].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 12a, string was " + s, "pt1, pt2, pt3'", s);
		s = p.getField(1, lines2[2].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 12b, string was " + s, "pt1| pt2| pt3'", s);

		s = p.getField(1, lines1[3].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 13a, string was " + s, "pt1, 'pt2' , pt3", s);
		s = p.getField(1, lines2[3].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 13b, string was " + s, "pt1| 'pt2' | pt3", s);

		s = p.getField(1, lines1[4].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 14a, string was " + s, "pt1, 'pt2' , 'pt3'", s);
		s = p.getField(1, lines2[4].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 14b, string was " + s, "pt1| 'pt2' | 'pt3'", s);

		s = p.getField(1, lines1[5].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 15a, string was " + s, "pt1, 'pt2', 'pt3', pt4", s);
		s = p.getField(1, lines2[5].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 15b, string was " + s, "pt1| 'pt2'| 'pt3'| pt4", s);

		s = p.getField(1, lines1[6].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 16a, string was " + s, "'f2a',' f2b'", s);
		s = p.getField(1, lines2[6].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 16b, string was " + s, "'f2a'|' f2b'", s);

		s = p.getField(1, lines1[7].getBytes(), new CsvDefinition("," , "'"));
		assertEqualsV3o("Error in 17a, string was " + s, "pt1, 'pt2' , 'pt3', pt4", s);
		s = p.getField(1, lines2[7].getBytes(), new CsvDefinition("|" , "'"));
		assertEqualsV3o("Error in 17b, string was " + s, "pt1| 'pt2' | 'pt3'| pt4", s);
	}

	@Test public void testSetFieldA() {
		ICsvByteLineParser p = new BasicCsvByteLineParserExtended(false);
		tstSetField(p);
	}

	@Test public void testSetFieldB() {
		ICsvByteLineParser p = new BasicCsvLineParserExtended(false);
		tstSetField(p);
	}
		...
		for (int i = 0; i < lines1.length - 1; i++) {
			for (j = 0; j < 3; j++) {
				s = p.setFieldByteLine(j, Type.NT_NUMBER, 
						...
						p.getField(j, lines1[i].getBytes(), new CsvDefinition(",", "'")));
				assertEqualsV3o("Error in " + i +":" + j + "a got " + new String(s), lines1[i], new String(s));

				s = p.setFieldByteLine(j, Type.NT_NUMBER, lines2[i].getBytes(), new CsvDefinition("|", "'"),
						p.getField(j, lines2[i].getBytes(), new CsvDefinition("|", "'")));
				assertEqualsV3o("Error in " + i +":" + j + "b got " + new String(s), lines2[i], new String(s));

				s = p.setFieldByteLine(j, Type.NT_NUMBER, lines0[i].getBytes(), new CsvDefinition(",", "'"),
						p.getField(j, lines0[i].getBytes(), new CsvDefinition(",", "")));
				assertEqualsV3o("Error in " + i +":" + j + "c got " + new String(s), lines0[i], new String(s));
			}
		}
	}


	@Test public void testSetField3a() {
			BasicCsvByteLineParserExtended p = new BasicCsvByteLineParserExtended(false, ICsvDefinition.NORMAL_SPLIT, false, true);
			tstSetField3(p);
	}

	@Test public void testSetField3b() {
		...
		for (int i = 0; i < lines1.length - 1; i++) {
			for (j = 0; j < 3; j++) {
				...
				if (before.indexOf(",") == 0) {
					...
					assertEqualsV3o("Error in " + i +":" + j + "aa got " + after, after, "'" + before + "'");
				}
				after = p.getField(j, s, csvDefCommaSingleQuote);
				assertEqualsV3o("Error in " + i +":" + j + "ab got " + after, before, after);
				...
				if (before.indexOf("|") == 0) {
					...
					assertEqualsV3o("Error in " + i +":" + j + "ba got " + after, after, "'" + before + "'");
				}
				after = p.getField(j, s, csvDefBarSingleQuote);
				System.out.println(i + ", " + j + " > " + before + " => " + after);
				assertEqualsV3o("Error in " + i +":" + j + "bb got " + after, before, after);
				...
				assertEqualsV3o("Error in " + i +":" + j + " ca got " + after, "'" + before + "'", after);
				after = p.getField(j, s, csvDefCommaSingleQuote);
				assertEqualsV3o("Error in " + i +":" + j + "cb got " + after, before, after);
				System.out.println();
			}
		}
	} 
	...
	@Test public void testGetFieldList() {
	...
	@Test public void testSetFieldList() {
		BasicCsvByteLineParserExtended p = new BasicCsvByteLineParserExtended(false);
