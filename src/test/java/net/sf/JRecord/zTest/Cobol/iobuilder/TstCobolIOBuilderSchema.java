...
public class TstCobolIOBuilderSchema  {
	 ...
	 *  
	 */
	@Test public void test01() throws RecordException, IOException {
	 ...
	 */
	@Test public void test02() throws RecordException, IOException {
	 ...
	 */
	@Test public void test03() throws RecordException, IOException {
	...
	private void tstBldr(ICobolIOBuilder ioBuilder, TstData data) throws RecordException, IOException {
		...
		assertEquals( data.fileOrganization, schema.getFileStructure(), data.testId);
		if (data.font == null 
		...
		assertEquals( data.expectedFields.length, schema.getRecordCount(), data.testId);
		
		for (int i = 0; i < schema.getRecordCount(); i++) {
			t1 = data.testId + ": " + i;
			rec = schema.getRecord(i);
			
			assertEquals( data.expectedFields[i].length, rec.getFieldCount(), t1);

			for (int j = 0; j < rec.getFieldCount(); j++) {
				t2 = t1 + ", " + j;
				fld = rec.getField(j);
				ef =  data.expectedFields[i][j];
				assertEquals( ef.getName(), fld.getName(), t2);
				assertEquals( ef.getPos(),  fld.getPos(), t2);
				assertEquals( ef.getLen(),  fld.getLen(), t2);
				assertEquals( ef.getDecimal(), fld.getDecimal(), t2);
				int shortType = typeMgr.getShortType(ef.getType(), ef.getLen(), charsetType);
				if (shortType !=  fld.getType()) {
					assertEquals( shortType, fld.getType(), t2);
				}
				
				assertEquals( ef.getFontName(), fld.getFontName(), t2);
			}
		}
	}
