...
public class TstSetStartingPosition  {
	 ...
	 * @throws IOException
	 */
	@Test public void testBuildUsingStream() throws IOException {
	 ...
	 * @throws IOException
	 */
	@Test public void testBuildUsingReader() throws IOException {
	 ...
	 * @throws IOException
	 */
	@Test public void testBuildUsingFile() throws IOException {
	...
	private void checkXRec(ExternalRecord xr, int pos) {
		...
		for (int i = 0; i < dtar020Positions.length; i++) {
			assertEquals(dtar020Positions[i], record.getRecordField(i).getPos(), pos + ": " + i);
		}
		
		record = xr.getRecord(1);
		for (int i = 0; i < recordAPositions.length; i++) {
			assertEquals( recordAPositions[i] + p, record.getRecordField(i).getPos(), pos + ": " + i);
		}
	}
	 ...
	 * @throws IOException
	 */
	@Test public void testFieldBuildUsingStream() throws IOException {
	 ...
	 * @throws IOException
	 */
	@Test public void testFieldBuildUsingReader() throws IOException {
		String zOther = "ZOther";
		ICobolMultiCopybookIOBuilder iob = JRecordInterface1.COBOL
				.newMultiCopybookIOBuilder("MultiCopybook")
					.setDialect(ICopybookDialects.FMT_MAINFRAME)
	 ...
	 * @throws IOException
	 */
	@Test public void testFieldBuildUsingFile() throws IOException {
	...
	private void checkXRec2(ExternalRecord xr, int idx, int [] fieldPositions) {
		...
		for (int i = 0; i < fieldPositions.length; i++) {
			assertEquals( fieldPositions[i] + p, record.getRecordField(i).getPos(),": " + i);
		}
	}
