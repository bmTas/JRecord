 ...
 */
	...
	@Test public void testOpenCobol() throws Exception {
		checkFiles("default/", "cpy", ICopybookDialects.FMT_GNU_COBOL);
	}
	
	
	@Test public void testOpenCobolBS2000() throws Exception {
		checkFiles("bs2000/", "bs", ICopybookDialects.FMT_FS2000);
	}
	
	
	@Test public void testOpenCobolMf() throws Exception {
		checkFiles("mf/", "mf", ICopybookDialects.FMT_OC_MICRO_FOCUS);
	}

	
	@Test public void testOpenCobolMvs() throws Exception {
		checkFiles("mvs/", "mvs", ICopybookDialects.FMT_GNU_COBOL_MVS);
	}

	
	@Test public void testOpenCobolFj() throws Exception {
		...
		while ((line = reader.read()) != null) {
			...
			for (i = st; i < len.length; i++) {
				if (len[i] > log) {
					...
					assertEquals(refVal, cmp, "Error Line: " + j + " " + vars[i]);
				}
			}
		}
