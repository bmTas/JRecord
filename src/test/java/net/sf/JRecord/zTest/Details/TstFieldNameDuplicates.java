...
public class TstFieldNameDuplicates  {
	...
	@Test public void testFieldNames() throws RecordException {
...
//		}
		...
		for (String s : keySet) {
			...
			assertEquals( fieldNames[i], s, "check Field Name");
			assertEquals( fieldDetails[i][0], fieldDetail.getPos());
			assertEquals( fieldDetails[i][1], fieldDetail.getLen());
			assertEquals( fieldDetails[i][2], fieldDetail.getType());
			i += 1;
		}
