...
public class TstNumericManager  {

	@Test public void testManagerVbFileStructure() {
		int[] openCob = {
		...
		for (int i = 0; i < openCob.length; i++) {
			...
			assertEquals(
					...
					"GNU Cobol " + c.getName() + " " + i);
		}
		assertEquals( Constants.IO_VB, m.getConverter4code(ICopybookDialects.FMT_MAINFRAME).getFileStructure(true, true),
				"Mainframe");
		assertEquals( Constants.IO_VB_FUJITSU, 
				m.getConverter4code(ICopybookDialects.FMT_FUJITSU).getFileStructure(true, true),
				"Fujitsu");
	}
