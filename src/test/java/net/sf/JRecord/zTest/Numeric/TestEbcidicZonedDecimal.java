...
public class TestEbcidicZonedDecimal extends JUnit3Test  {
	...
	@Test public void testZonedSetValue() throws RecordException {
		checkType(Type.ftZonedNumeric);
	}
	
	@SuppressWarnings("deprecation")
	@Test public void testSmallZonedSetValue() throws RecordException {
	...
	private void tstValue(FieldDetail fld, int val, byte highNyble) throws RecordException {
		...
		org.junit.jupiter.api.Assertions.assertEquals(
				highNyble, (byte) (b[fldEnd] & HIGH_NYBLE), "Test Sign: " +id);
		org.junit.jupiter.api.Assertions.assertEquals( 
				absVal % 10,  (b[fldEnd] & LOW_NYBLE), "Test last char: "  +id);
		
		for (int j = 1; j <= fldEnd; j++) {
			absVal = absVal / 10;
			org.junit.jupiter.api.Assertions.assertEquals(
					absVal % 10,  (b[fldEnd - j] & LOW_NYBLE), "Test digit: " + (j + 1) + " in "  +id);
			org.junit.jupiter.api.Assertions.assertEquals( 
					((byte) 0xF0), (byte) (b[fldEnd - j] & HIGH_NYBLE), "Test nyble: " + id);
		}

		Object fldValue = tz.getField(b, 1, fld);
//		System.out.println("-->" + fldValue + "<--" + fldValue.getClass().getName());
		assertEqualsV3o("Check Value: "  +id, Integer.toString(val), fldValue.toString());
...
//
//	
//	@Test public void testZonedLeadingEbcdic() {
//		
//		for (String charset : CHARSET_TO_TEST) {
//			System.out.println(">> " + charset + " ");
