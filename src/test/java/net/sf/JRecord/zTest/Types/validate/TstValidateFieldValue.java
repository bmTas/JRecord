...
public class TstValidateFieldValue extends JUnit3Test  {
	...
	protected void tstMethod(LayoutDetail schema, AbstractLine line) {
		...
		for (int fldNum = 0 ; fldNum < fieldCount; fldNum++) {
			...
			if ((!TypeManager.isBinary(field.getType())) || fldNum > 3) {
				assertFalseV3("Field-Number: " + fldNum, line.getFieldValue(0, fldNum).isValid());
				assertFalseV3("Field-Number: " + fldNum, line.getFieldValue(field).isValid());
			}
		}
	}
