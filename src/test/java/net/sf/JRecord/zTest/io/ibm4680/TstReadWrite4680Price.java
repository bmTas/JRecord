 ...
 *
 */
public class TstReadWrite4680Price extends JUnit3Test   {
	...
	@Test public void testPriceRead1() throws Exception {
	...
	@Test public void testPriceRead2() throws Exception {
			...
			if (pref < 0) {
				...
				for (int j = 0; j < expectedLines[i].length ; j++) {
					if (expectedLines[i][j] != null) {
						assertEqualsV3o(i + ", " + j + " ~ " + l.getFullLine(), expectedLines[i][j], l.getFieldValue(pref, j).asString());
					}
				}
			}
			i += 1;
	...
	private void tstWrite(String filename, int fileStructure, int expected) throws Exception {
		...
		for (i = 0; i < lines.size(); i++) {
			...
			for (int j = 0; j < lineAsBytes.length; j++) {
				assertEqualsV3(i+", " + j + ", " + p, lineAsBytes[j], bytes[p++]);
			}
		}
	}
