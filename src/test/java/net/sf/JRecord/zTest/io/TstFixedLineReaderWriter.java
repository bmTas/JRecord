...
public class TstFixedLineReaderWriter extends JUnit3Test  {
	...
	@Test public void testWrite1() throws IOException, RecordException {
	...
	@Test public void testWrite2() throws IOException, RecordException {
		
		for (int k = 0; k < TEST_SIZES.length; k++) {
			chkWrite(TEST_SIZES[k], buildByteArray2(k));
		}
	}
	@Test public void testWrite3() throws IOException, RecordException {
		
		for (int k = 0; k < TEST_SIZES.length; k++) {
			chkWrite(TEST_SIZES[k], buildByteArray3(k));
		}
	}
	@Test public void testWrite4() throws IOException, RecordException {
	...
	private void chkWrite(int size, byte[] b) {
		...
		for (int i = 0; i < WRITE_COUNT; i++) {
			...
			for (int j = st; j < en1; j++) {
				assertEqualsV3("Tst1: " + size + " - " + i + ", " + j,
						expected[j-st], 
						b[j]);
			}
			for (int j = en1; j < en2; j++) {
				if (b[j] != 32) { //Space
					assertEqualsV3i("Tst2: " + size + " - "+ i + ", " + j, 0, b[j]);
				}
			}
		}
	}
	...
	@Test public void testRead() throws IOException, RecordException { 
