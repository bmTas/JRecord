...
public class CharLine2 extends BasicLine implements Cloneable {
	...
	public CharLine2(LayoutDetail layoutDef, String line) {
	...
	@Override
	public byte[] getData() {
		return Conversion.getBytes(subString(0, dataLength), layout.getFontName());
	}
	...
	public Object getField(int typeId, IFieldDetail field) {
		...
		if (field.isFixedFormat()) {
			...
			return layout.formatCsvField(field, typeId, subString(0, dataLength));
		}
	}
	...
	private String getFieldRawText(IFieldDetail fldDef) {
		...
		if (fldDef.getType() == Type.ftCharRestOfRecord || tempLen > dataLength - start) {
			tempLen = dataLength - start;
		}

	    if (dataLength < start || tempLen < 1 || start < 0) {
	        return "";
	    }
		return subString( start, tempLen);
				//tData.substring(start, start + tempLen);
	}


	@Override
	public String getFullLine() {
		return subString(0, dataLength);
	}
		...
		if (ret == Constants.NULL_INTEGER) {
			...
			if (ret < 0) {
				for (int i=0; i< layout.getRecordCount(); i++) {
					if (this.dataLength == layout.getRecord(i).getLength()) {
						ret = i;
						preferredLayout = i;
						break;
					}
			...
			if (layout != null) {
				if (layout.isCsvLayout()) {
					...
					if (layout.getRecordCount() > 0) {
						recLength = Math.max(recLength, layout.getRecord(0).getFieldCount() * 3);
					}
				} else 
					recLength = layout.getMinimumRecordLength();
			}
			data = new char[recLength];
			Arrays.fill(data, ' ');
			dataLength = 0;
		} else {
	...
	private void updateData(int pos, int length, String value) {
		...
		dataLength = Math.max(dataLength, start + length);
	}
	...
	public boolean isDefined(IFieldDetail field) {
		if (this.data == null || dataLength <= field.getPos() || data.length <= field.getPos()) {
			return false;
		}

		if (TypeManager.isNumeric(field.getType())) {
			int e = Math.min(field.getPos() + field.getLen(), dataLength);
	...
	private String subString(int start, int count) {
		int strLength = Math.min(count, dataLength - start);
		return strLength > 0 ? new String(data, start, strLength) : "";
	}
